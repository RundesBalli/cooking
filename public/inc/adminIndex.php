...
"</div>";
$content.= "<div class='row hover bordered'>".
"<div class='col-s-12 col-l-3'>Anzahl Aufrufe</div>".
"<div class='col-s-12 col-l-9'>".$row['clickCount']."</div>".
"<div class='col-s-12 col-l-0'><div class='spacer-s'></div></div>".
"</div>";
$content.= "<div class='row hover bordered'>".
"<div class='col-s-12 col-l-3'>Aufrufe heute</div>".
...
/**
 * Abrufen uns Ausgeben des "mostClicked" SQL Views zur Anzeige der meist aufgerufenen Rezepte.
 */
$content.= "<h2><span class='far icon'>&#xf25a;</span>Am meisten aufgerufene Rezepte</h2>";
$result = mysqli_query($dbl, "SELECT * FROM `mostClicked`") OR DIE(MYSQLI_ERROR($dbl));
if(mysqli_num_rows($result) == 0) {
  /**
   * Wenn noch keine Aufrufe vorhanden, dann kurze Info.
   */
  $content.= "<div class='infobox'>Noch keine Aufrufe.</div>";
} else {
  /**
   * Wenn Aufrufe vorhanden sind, dann erfolgt die Auflistung.
   */
  $top = 0;
  $content.= "<div class='row highlight bold'>".
  "<div class='col-s-12 col-l-3'>Platz / Aufrufe</div>".
  ...
  while($row = mysqli_fetch_assoc($result)) {
    $top++;
    $content.= "<div class='row hover bordered'>".
    "<div class='col-s-12 col-l-3'><span class='highlight'>#".$top."</span> (".number_format($row['c'], 0, ",", ".")." Aufrufe)</div>".
