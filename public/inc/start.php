...
if(mysqli_num_rows($result) == 0) {
  ...
  while($row = mysqli_fetch_array($result)) {
    ...
    $content.= "<a href='/kategorie/".$row['shortTitle']."' style='background-image: linear-gradient(0deg, rgba(22,22,24, 0.7), rgba(22,22,24, 0.7)), url(\"".$thumb."\");'><div class=\"category-wrap\">".output($row['title']).(!empty($row['shortDescription']) ? "<span>".output($row['shortDescription'])."</span>" : NULL)."</div></a>".PHP_EOL;
  }
  $content.= "</div>".PHP_EOL;
}
?>
